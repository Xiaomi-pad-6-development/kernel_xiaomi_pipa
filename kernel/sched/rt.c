		  ...
		  int sibling_count_hint)
{
	...
	test = unlikely(rt_task(curr)) && (curr->nr_cpus_allowed < 2 || curr->prio <= p->prio);
	if (static_branch_unlikely(&sched_energy_present) || may_not_preempt || test || !rt_task_fits_capacity(p, cpu)) {
